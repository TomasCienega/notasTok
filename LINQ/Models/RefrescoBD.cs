...
namespace ClasesyObjetos.Models
{
    internal class RefrescoBD
    {
        ...
        public void Add(Refresco refresco)
        {
            string query = "insert into refresco(nombre, marca, gas, cantidad) " +
                "values(@nombre, @marca, @gas, @cantidad)";

            using (var connection = new SqlConnection(cadena))
            {
                var command = new SqlCommand(query, connection);
                command.Parameters.AddWithValue("@nombre",refresco.Nombre);
                command.Parameters.AddWithValue("@marca", refresco.Marca);
                command.Parameters.AddWithValue("@gas", refresco.Gas);
                command.Parameters.AddWithValue("@cantidad", refresco.Cantidad);
        ...
        public void Edit(Refresco refresco, int Id)
        {
            string query = "update refresco set nombre=@nombre, marca=@marca, gas=@gas, cantidad=@cantidad " +
                "where id=@id";
        ...
        {
            string query = "delete from refresco " +
                "where id=@id";
